...
This module defines:
- The SQLAlchemy ORM model for persisting Synology data.
- The Pydantic schema for validating API requests when creating a Synology.
...
class Synology(Base):
    """
    SQLAlchemy ORM model representing a Synology record.
    ...
    def __repr__(self):
        """
        Returns a string representation of the Synology instance.

        Example:
            <Synology(id=1, username='johndoe', email='john@example.com')>
        """
        return f"<Synology(id={self.id}, username='{self.username}', email='{self.email}')>"


class SynologyCreate(BaseModel):
    """
    Pydantic schema for creating a new Synology.
